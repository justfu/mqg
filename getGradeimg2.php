...
function query4($cookie){
    $starttime=microtime(true);
    $url = "jwmis.hnie.edu.cn/jwweb/XSCJ/Stu_MyScore_print_rpt.aspx?xn=2015&xq=1&rpt=1&rad=2&zfx=0";
    $ch = curl_init($url);
    // curl_setopt($ch, CURLOPT_POST, true);
    // curl_setopt($ch, CURLOPT_POSTFIELDS, $arr);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
function query5($cookie){
    $starttime=microtime(true);
    $url = "http://jwmis.hnie.edu.cn/jwweb/XSCJ/Stu_MyScore_Drawimg.aspx?x=1&h=2&w=740&xnxq=20151&xn=2015&xq=1&rpt=1&rad=2&zfx=";
    $ch = curl_init($url);
    // curl_setopt($ch, CURLOPT_POST, true);
    // curl_setopt($ch, CURLOPT_POSTFIELDS, $arr);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    // echo curl_error($ch);
    curl_close($ch);
    if(strpos($data,'500')>0){
      echo "<h2 style='color:skyblue;text-align:center;'>本学期详细成绩暂未公布,查看部分已公布的成绩..请戳 **本学期成绩分布** 查看  同时 望各位考试顺利！！O(∩_∩)O~~</h2>";
      exit;
    }
